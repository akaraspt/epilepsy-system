function [] = stackedAutoencodersCostTest()

	% Check the gradients for the stacked autoencoder
	%
	% In general, we recommend the creation of such files for checking
	...
	[cost, grad] = stackedAutoencodersCost(stackedAETheta, inputSize, hiddenSize, ...
								 numClasses, netconfig, ...
								 lambda, data, labels);

	% Check that the numerical and analytic gradients are the same
	numgrad = computeNumericalGradient( @(x) stackedAutoencodersCost(x, inputSize, ...
