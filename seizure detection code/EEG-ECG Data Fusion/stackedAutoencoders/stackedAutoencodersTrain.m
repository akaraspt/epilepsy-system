function [stackedAutoencoders, netconfig] = stackedAutoencodersTrain( ...
	...
	if strcmp(minFuncOptionsFinetuning.Method,'lbfgs')
	...
	elseif strcmp(minFuncOptionsFinetuning.Method,'sgd')
		[stackedAutoencoders.optTheta, cost] = sgd(@(p,D,d) stackedAutoencodersCost(p, ...
			layerSizes{1}, layerSizes{end}, numClasses, netconfig, ...
			hyperParamsFinetuning.lambda, D, d), stackedAutoencoders.theta, ...
			minFuncOptionsFinetuning, X(:,1:numTrain), y);
	end
end