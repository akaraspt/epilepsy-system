...
from pylearn2ext.epilepsiae import Epilepsiae
...
def predict_sdae_leave_one_out_chbmit(channel_thresholds, model_path, data_path):
    """
    Script to evaluate the performance with leave-one-file-out cross validation for CHBMIT patients.
    ...
    for patient_id in patients:
        with open(os.path.join(model_path, 'sdae_chbmit_performance_p{0}'.format(patient_id)), 'wb') as f:
    ...
    save_performance_path = model_path
    with open(os.path.join(save_performance_path, 'sdae_chbmit_performance_per_patient'), 'wb') as f:
        f.write('patient_id,threshold,accuracy,mean_latency,fp_per_hour,fp_range_per_hour\n')

        for patient_id in patients:
            perf_df = pd.read_csv(os.path.join(save_performance_path, 'sdae_chbmit_performance_p{0}'.format(patient_id)))
            sum_perf_df = perf_df.groupby('threshold').sum()

            print ''
            print '====== [Summary of Patient ' + str(patient_id) +'] ======'
            print ''

            for idx, row in sum_perf_df.iterrows():
                accuracy = row['tp'] / row['total_seizures']
                latency = row['mean_latency'] / sum_perf_df.shape[0]
                fp_per_hour = row['fp'] / row['total_hours']
                fp_range_per_hour = row['fp_range'] / row['total_hours']
                print 'channel_threshold={0}: accuracy={1}, latency={2}, fp_per_hour={3}, fp_range_per_hour={4}'.format(idx, accuracy, latency,
                                                                                                                        fp_per_hour, fp_range_per_hour)
                f.write('{0},{1},{2},{3},{4},{5}\n'.format(patient_id,idx, accuracy, latency,
                                                           fp_per_hour, fp_range_per_hour))

    all_perf_df = pd.read_csv(os.path.join(save_performance_path, 'sdae_chbmit_performance_per_patient'))
    mean_all_perf_df = all_perf_df.groupby('threshold').mean()

    print ''
    print '====== [Summary of all Patient] ======'
    print ''

    with open(os.path.join(save_performance_path, 'sdae_chbmit_performance_all_patient'), 'wb') as f:
        f.write('threshold,accuracy,mean_latency,fp_per_hour,fp_range_per_hour\n')

        for idx, row in mean_all_perf_df.iterrows():
            print 'channel_threshold={0}: accuracy={1}, latency={2}, fp_per_hour={3}, fp_range_per_hour={4}'.format(idx,
                                                                                                                    row['accuracy'],
                                                                                                                    row['mean_latency'],
                                                                                                                    row['fp_per_hour'],
                                                                                                                    row['fp_range_per_hour'])
            f.write('{0},{1},{2},{3},{4}\n'.format(idx,
                                                   row['accuracy'],
                                                   row['mean_latency'],
                                                   row['fp_per_hour'],
...
if __name__ == '__main__':
    ...
    channel_thresholds = np.asarray([1, 3, 5])
    data_path = '/Users/akara/Workspace/data/chbmit'
    model_path = '../models'

    predict_sdae_leave_one_out_chbmit(channel_thresholds=channel_thresholds,
                                      data_path=data_path,
