function [qrs_amp_raw,qrs_i_raw,delay]=pan_tompkin(ecg,fs,thre,gr)
%% modified version of pan-tomkins algorithm
...
if gr
ax(6)=subplot(326);plot(ecg_m);axis tight;title('Averaged with 30 samples length,Black noise,Green Adaptive shold,RED Sig Level,Red circles QRS adaptive threshold');
axis tight;
end
...
[pks,locs] = findpeaks(ecg_m,'MINPEAKDISTANCE',round(0.4*fs));
          ...
          if (locs(i) - qrs_i(end)) >= round(1.66*test_m)% it shows a QRS is missed 
              ...
              if pks_temp > THR_NOISE
               ...
               if y_i_t > THR_NOISE1 
                      ...
                      %%yan
                      if length(qrs_amp_raw) == 1
                          SIG_LEV1 = 0.25*y_i_t + 0.75*SIG_LEV1; %when found with the second thres 
                      elseif length(qrs_amp_raw) > 1
                          if y_i_t <= thre*qrs_amp_raw(end-1)
    ...
    if pks(i) >= THR_SIG
        ...
        % bandpass filter check threshold
         if y_i >= THR_SIG1
                     ...
                     %%yan
                      if length(qrs_amp_raw) == 1
                          SIG_LEV1 = 0.125*y_i + 0.875*SIG_LEV1; %when found with the second thres 
                      elseif length(qrs_amp_raw) > 1
                          if y_i <= thre*qrs_amp_raw(end-1)
...
%               SIG_LEV1 = 0.125*y_i + 0.875*SIG_LEV1;% adjust threshold for bandpass filtered sig
        ...
        % adjust Signal level
         %%yan
               if length(qrs_c) == 1
                    SIG_LEV = 0.125*pks(i) + 0.875*SIG_LEV ;  %when found with the second threshold             
               elseif length(qrs_c) > 1
                   if pks(i) <= thre*qrs_c(end-1)
